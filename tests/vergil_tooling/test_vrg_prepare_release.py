"""Tests for vergil_tooling.bin.st_prepare_release."""
...
from vergil_tooling.bin.st_prepare_release import (
    RELEASE_NOTES_DIR,
    _detect_cargo,
...
def test_ensure_on_develop_ok() -> None:
    mock = "vergil_tooling.bin.st_prepare_release.git.current_branch"
...
def test_ensure_on_develop_wrong_branch() -> None:
    with (
        patch(
            "vergil_tooling.bin.st_prepare_release.git.current_branch",
...
def test_ensure_clean_tree_ok() -> None:
    with patch("vergil_tooling.bin.st_prepare_release.git.read_output", return_value=""):
        _ensure_clean_tree()


def test_ensure_clean_tree_dirty() -> None:
    with (
        patch("vergil_tooling.bin.st_prepare_release.git.read_output", return_value="M file.py"),
...
def test_ensure_develop_up_to_date_ok() -> None:
    with (
        patch("vergil_tooling.bin.st_prepare_release.git.run"),
        patch("vergil_tooling.bin.st_prepare_release.git.read_output", return_value="abc123"),
    ):
        _ensure_develop_up_to_date()
...
def test_ensure_develop_up_to_date_diverged() -> None:
    ...
    with (
        patch("vergil_tooling.bin.st_prepare_release.git.run"),
        patch(
            "vergil_tooling.bin.st_prepare_release.git.read_output",
...
def test_main_full_flow(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with (
        ...
        patch("vergil_tooling.bin.st_prepare_release.git.ref_exists", return_value=False),
        patch(
            "vergil_tooling.bin.st_prepare_release.subprocess.run",
            side_effect=mock_subprocess_run,
        ),
        patch(
            "vergil_tooling.bin.st_prepare_release.github.create_pr",
...
def test_main_release_branch_already_exists(
    ...
    with (
        ...
        patch("vergil_tooling.bin.st_prepare_release.git.ref_exists", return_value=True),
