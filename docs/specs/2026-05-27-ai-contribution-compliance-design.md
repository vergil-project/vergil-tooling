...
| P4: Disclosure readiness | **Enforced** | `vrg-commit --agent` requires AI agent declaration. `Co-Authored-By` trailer is mandatory on all AI-assisted commits. Every commit in git history is attributable to its AI tool. | `vrg-commit` (agent flag, co-author resolution), `VRG_CO_AUTHOR` env var |
