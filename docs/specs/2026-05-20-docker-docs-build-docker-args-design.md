# Design: Eliminate hardcoded mount in vrg-container-docs
...
`vrg_container_docs.py` builds its Docker argument list from scratch
...
Refactor `vrg_container_docs.py` to delegate to `build_docker_args()`
