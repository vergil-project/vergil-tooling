...
**Changes:**
1. Replace the `from vergil_tooling.lib.lima import (…)` block: keep the lifecycle primitives still imported from `lima` (`create_vm`, `start_vm`, `stop_vm`, `delete_vm`, `fetch_template`, `get_tooling_version`?, `vm_status`, `list_vms`, `vm_age_days`, `nested_virt_unsupported_reason`, `copy_claude_config`?, `shell_run`). Import the relocated helpers from `vm_guest` (`inject_credentials`, `install_tooling`, `update_tooling`, `try_update_tooling`, `get_tooling_version`, `copy_claude_config`, `link_claude_dirs`, `vm_probe`, `vm_spec_status`).
